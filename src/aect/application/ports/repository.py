...
class RepositoryPort(Protocol):
    ...
    set_discontinued / set_discontinued_async (Monitoring, V4.1-S7): dediziertes
    Per-Feld-UPDATE (F-011-Muster, analog update_status) fuer das discontinued-
    Flag -- kein save() der ganzen Zeile, kein Lost-Update gegenueber parallelen
    LLM-Feld-Schreibvorgaengen. No-op, wenn case_id nicht existiert (analog
    delete/update_field).

    set_solution_refine_count / set_solution_refine_count_async: dediziertes
    Per-Feld-UPDATE fuer den vom Service fertig berechneten Refine-Zaehler.

    list_deleted / list_deleted_async + set_deleted_at / set_deleted_at_async
    (zweistufiges Loeschen, ADR-0057): der Papierkorb. list_all() liefert
    ausschliesslich AKTIVE Cases (deleted_at is None) -- das ist Teil des
    Port-Vertrags, nicht eine Eigenheit eines Adapters; list_deleted() liefert
    das Gegenstueck, zuletzt geloescht zuerst. set_deleted_at setzt den
    Zeitstempel (Papierkorb) bzw. None (Wiederherstellen) per dediziertem
    Per-Feld-UPDATE (F-011-Muster, analog set_discontinued). get() bleibt
    UNGEFILTERT -- ein Case im Papierkorb muss per ID ladbar bleiben, sonst
    haette weder das Wiederherstellen noch das endgueltige Loeschen etwas zu
    greifen. No-op bei unbekannter case_id (analog delete/update_field).
    """
    ...
    async def set_discontinued_async(
        self, case_id: str, discontinued: bool
    ) -> None: ...
    async def set_solution_refine_count_async(
        self, case_id: str, count: int
    ) -> None: ...
    async def list_deleted_async(self) -> list[SubmittedCase]: ...
    async def set_deleted_at_async(
        self, case_id: str, value: datetime | None
    ) -> None: ...
